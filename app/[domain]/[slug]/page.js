        ...
        <article className="px-6 py-10 md:px-12 md:py-16 lg:px-20 max-w-4xl">
            <header className="mb-12">
                <nav className="mb-8 text-xs uppercase tracking-widest text-black/60 font-mono">
                    <Link href={`/${domain}`} className="hover:text-accent-red">
                        {domain.replace(/-/g, " ")}
                    </Link>
                ...
                <h1 className="text-3xl md:text-5xl font-serif font-bold mb-8 leading-tight tracking-tight">
                    {entry.title}
                </h1>

                {entry.date && (
                    <div className="font-mono text-sm tracking-wide mb-12 border-b border-border-subtle pb-4">
                        [{entry.date}]
                    </div>
                )}
            </header>
