...
export default async function DomainPage({ params }) {
  ...
  const intro = await getIntro(domain);
            ...
            <article key={entry.slug} className="bg-card-bg p-8 md:p-10 lg:p-12">
              <div className="flex flex-col md:flex-row gap-6 md:gap-12">
                <div className="w-full md:w-[160px] shrink-0">
                  <span className="font-mono text-base md:text-lg text-black block">[{entry.date || 'N/A'}]</span>
                </div>
                <div className="flex-1">
                  <h3 className="text-xl md:text-3xl font-bold leading-tight mb-4 tracking-tight font-sans text-black">{entry.title}</h3>
                  <p className="text-base md:text-lg leading-relaxed mb-6 max-w-prose text-black">{entry.description}</p>
                  <Link href={`/${domain}/${entry.slug}`} className="inline-flex items-center text-sm font-semibold uppercase tracking-wider hover:underline underline-offset-4 group text-black">
                    BACA SELENGKAPNYA
